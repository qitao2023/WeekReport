...
class API:
    def get_defaults(self):
        ...
        return json.dumps({
            'log': str(DATA_DIR / '工作日志 -2026.txt'),
            'out': str(DATA_DIR / '齐涛'),
    ...
    def get_week_options(self, log_path, out_dir):
        ...
        try:
            log = Path(log_path)
            out = Path(out_dir)
            if not log.is_file(): return json.dumps([])
            entries = parse_log(str(log))
            groups = group_by_project_week(entries)
            existing = set()
            if out.is_dir():
...
body{font-family:-apple-system,"PingFang SC",sans-serif;background:#f5f5f7;padding:24px 20px;overflow:hidden;-webkit-app-region:drag;user-select:none}
...
.input-group input{flex:1;padding:8px 10px;font-size:12px;font-family:"SF Mono",Menlo,monospace;border:1px solid #d1d1d6;border-radius:6px;outline:none;background:#fff;-webkit-app-region:no-drag}
...
<div class="row"><label>选择周次</label><select id="weekSelect" onchange="onWeekChange()"></select></div>
